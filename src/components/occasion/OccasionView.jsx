      ...
      <main className="flex-1 p-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-stone-500">Manage and view occasions list</p>
          </div>
        ...
        <div className="rounded-2xl border border-stone-200 bg-white shadow-sm">
          ...
          <div className="border-b border-stone-200 px-6 py-4">
            <h2 className="text-base font-semibold text-stone-800">Occasion List</h2>
          </div>
                ...
                {loading ? (
                  ...
                  filteredItems.map((item, index) => (
                    <tr key={item.id} className="hover:bg-amber-50/40 transition-colors">
                      ...
                      {visibleCols.name && (
                        <td className="px-6 py-4 font-medium text-stone-800">{item.occasions}</td>
                      )}
                      {visibleCols.status && (
                        <td className="px-6 py-4">
                          <span
                            className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold border ${
                              item.occasions_status === 'Active'
                                ? 'bg-amber-50 text-amber-800 border-amber-300'
                                : 'bg-stone-100 text-stone-500 border-stone-200'
                            }`}
                          >
                            <span className={`h-1.5 w-1.5 rounded-full ${
                              item.occasions_status === 'Active' ? 'bg-amber-500' : 'bg-stone-400'
                            }`} />
                            {item.occasions_status || 'Active'}
                          </span>
                        </td>
                      ...
                      <td className="px-6 py-4">
                        <button
                          ...
                          title="Edit Occasion"
                          aria-label={`Edit ${item.occasions}`}
                        >
                          <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <tr>
                    ...
                    >
                      <div className="flex flex-col items-center gap-2 text-stone-400">
                        ...
                        {searchQuery ? `No occasions match "${searchQuery}"` : 'No occasions found.'}
                      </div>
                    </td>
                  </tr>
                )}
          ...
          <div className="flex items-center justify-between border-t border-stone-100 px-6 py-4">
            <p className="text-sm text-stone-500">
              ...
              <span className="font-semibold text-stone-800">{items.length}</span> occasions
            </p>
            <div className="flex items-center gap-2">
              <button
                type="button"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-stone-200">
            ...
            <form onSubmit={onSubmit} className="space-y-5">
              {/* Occasion Name */}
              <div>
                <label htmlFor="occasions" className="mb-1.5 block text-sm font-medium text-stone-700">
                  Occasion Name
                </label>
                <input
                  id="occasions"
              ...
              {/* Status */}
              <div>
                <label htmlFor="occasions_status" className="mb-1.5 block text-sm font-medium text-stone-700">
                  Status
                </label>
                <select
                  id="occasions_status"
